package com.example.skalelit.data.database

import android.content.Context
import androidx.room.Database
import androidx.room.Room
import androidx.room.RoomDatabase
import androidx.room.TypeConverters
import com.example.skalelit.data.converters.Converters
import com.example.skalelit.data.dao.AppDao
import com.example.skalelit.data.entity.BookingEntity
import com.example.skalelit.data.entity.RoomEntity
import com.example.skalelit.data.entity.UserEntity

@Database(
    entities = [UserEntity::class, RoomEntity::class, BookingEntity::class],
    version = 5, // INCREASED VERSION NUMBER
    exportSchema = false
)
@TypeConverters(Converters::class)
abstract class AppDatabase : RoomDatabase() {
    abstract fun dao(): AppDao

    companion object {
        @Volatile
        private var INSTANCE: AppDatabase? = null

        fun getDatabase(context: Context): AppDatabase {
            return INSTANCE ?: synchronized(this) {
                val instance = Room.databaseBuilder(
                    context.applicationContext,
                    AppDatabase::class.java,
                    "skalelit_db"
                )
                    .fallbackToDestructiveMigration() // <--- THIS FIXES THE CRASH
                    .build()
                INSTANCE = instance
                instance
            }
        }
    }
}